...
public class CPP {
	...
	public int numOfStudentEnrolled(Coursee course, int trim, int day){
		...
		for (int i = 0; i < students.size(); i++) {
			if(students.get(i).getSchedule(trim) != null && students.get(i).getSchedule(trim).exist(course,day)){
				num++;
			}
		}
		return num;
	...
	public ClassRoom findProperRoom(int d, int p, int length, String tools){
		/*
		 * get room for a particular course requirements like gym
		 */
		if(tools == null){
			return findEmptyNormalRoom(d, p, length);
		}else{
			for (int i = 0; i < rooms.size(); i++) {
				if(rooms.get(i).getTools() != null && rooms.get(i).getTools().equals(tools) && rooms.get(i).isFree(d, p)){
					return rooms.get(i);
				}
			}
		}
	...
	public Teacher getSuggestedTeacher(int trim, int d, int per, Coursee course){
		/*
		 * get best teachers using getTeachScore() in Teacher.class
		 * then find the one with less courses to teach
		 * if teachers are equal choose anyone. 
		...
		int score = 0;
		for (int i = 0; i < best.length ; i++) {
			if(teachers.get(i).getTeachingScore(trim, d, per, course.getPeriodsADay(), course)>score){
				for (int j = 0; j < best.length; j++) {
					best[j] = null;
				}
				score = teachers.get(i).getTeachingScore(trim, d, per, course.getPeriodsADay(), course);
				best[i] = teachers.get(i);
			}else if(teachers.get(i).getTeachingScore(trim, d, per, course.getPeriodsADay(), course) == score){
				best[i] = teachers.get(i);
			}
		}
